import { M1_REQUEST_SEGMENT_KINDS } from
...
export function gatedResult(
  ...
  repetition: number,
  reason = "campaign_infrastructure_gate",
): M1V2RepetitionResult {
  return {
    armId,
    repetition,
    ...
    db: null,
    work: emptyWorkEvidence(),
  };
}

function emptyWorkEvidence(): M1V2RepetitionResult["work"] {
  return {
    observed: false,
    status: null,
    planRevision: null,
...
function aggregateArm(
  ...
  const segments = Object.fromEntries(M1_REQUEST_SEGMENT_KINDS.flatMap((kind) => {
