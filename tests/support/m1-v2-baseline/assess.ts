import { M1_REQUEST_SEGMENT_KINDS, type M1RequestSegmentKind } from
...
export function assessM1V2Repetition(
  ...
  const reducibleBytes = sum(attempts.map((attempt) =>
    sum(Object.entries(attempt.segments)
      .filter(([kind]) => POTENTIALLY_REDUCIBLE.has(kind as M1RequestSegmentKind))
      .map(([, bytes]) => bytes ?? 0))));
...
function summarizeAttempt(
  ...
  for (const event of segmentEvents) {
    ...
    if (!kind || !M1_REQUEST_SEGMENT_KINDS.includes(kind as M1RequestSegmentKind) || bytes === null) {
