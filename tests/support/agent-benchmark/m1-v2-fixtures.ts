...
import type { CanonicalM1V2Fixture, M1V2ArmId } from "./m1-v2-types.ts";
...
export function loadCanonicalM1V2Fixtures(repoRoot: string): CanonicalM1V2Fixture[] {
  return DEFINITIONS.map((definition) => {
    const path = join(
      repoRoot,
      "tests/support/agent-benchmark/fixtures/m1-v2",
      definition.file,
    );
    const scenario = JSON.parse(readFileSync(path, "utf8")) as CanonicalM1V2Fixture["scenario"];
    if (
