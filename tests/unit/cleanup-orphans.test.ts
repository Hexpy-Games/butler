...
test("cleanup-orphans only targets processes for the current Butler data root", () => {
  ...
  writeExecutable(
    ...
    [
      ...
      "set -euo pipefail",
      "if [ \"${1:-}\" = \"aux\" ]; then",
      "  printf '%s\\n' 'user 111 0.0 0.0 0 0 ?? S 10:00AM 0:00.00 /fake/bun run /shared/butler/packages/butler-agent/src/gateways/app/interface/cli/app-gateway-cli.ts --data /Users/example/.butler'",
      `  printf '%s\\n' 'user 222 0.0 0.0 0 0 ?? S 10:00AM 0:00.00 /fake/bun run ${currentData}/app/runtime/agent/versions/0.0.8/packages/butler-agent/src/gateways/app/interface/cli/app-gateway-cli.ts --data ${currentData}'`,
      "  exit 0",
      "fi",
      "if [ \"${1:-}\" = \"-o\" ] && [ \"${2:-}\" = \"etime=\" ]; then",
      "  printf '%s\\n' '10:00'",
      ...
      "  pid=\"${4:-}\"",
      "  case \"$pid\" in",
      "    111) printf '%s\\n' '/fake/bun run /shared/butler/packages/butler-agent/src/gateways/app/interface/cli/app-gateway-cli.ts --data /Users/example/.butler' ;;",
      `    222) printf '%s\\n' '/fake/bun run ${currentData}/app/runtime/agent/versions/0.0.8/packages/butler-agent/src/gateways/app/interface/cli/app-gateway-cli.ts --data ${currentData}' ;;`,
      "    *) printf '\\n' ;;",
      "  esac",
      "  exit 0",
      "fi",
