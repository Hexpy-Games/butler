...
import { openAIBoundedConversationSerializedBytes } from
...
test("accepted text result continues as a durable reference", () => {
  const root = mkdtempSync(join(tmpdir(), "butler-exact-replay-restart-"));
  const dbPath = join(root, "btcc.sqlite");
  const stores = openBtccSqliteStores({ dbPath, ownerId: "media" });
  try {
    const result = { ok: true, content: "M".repeat(16_000) };
    ...
    const first = replay.prepareMessages(messages, "round");
    expect(first).toEqual(messages);
    expect(stores.guidedToolJournal.findForTurn("turn", "result")?.deliveryState)
      .toBe("in_flight");
    expect(replay.prepareMessages(messages, "round")).toEqual(first);
    replay.failed("round");
    expect(replay.prepareMessages(messages, "round")).toEqual(first);
    replay.accepted("round", routeAccepted("round", "accepted"));

    const later = replay.prepareMessages(messages, "later");
    expect(later[0]?.operationResultReference?.version)
      .toBe("butler.operation-result-reference.v1");
    expect(Buffer.byteLength(later[0]!.content)).toBeLessThan(1024);
  } finally {
    stores.close();
    rmSync(root, { recursive: true, force: true });
  }
...
test("raw result stays byte-identical through same-cursor retry and resumes as a reference", async () => {
  const root = mkdtempSync(join(tmpdir(), "butler-exact-replay-route-"));
  const dbPath = join(root, "btcc.sqlite");
  const butlerData = join(root, "data");
  const result = { ok: true, content: "R".repeat(16_000) };
  const message = {
    role: "tool" as const,
    toolCallId: "result",
    name: "read_file",
  ...
  try {
    const first = openBtccSqliteStores({ dbPath, ownerId: "media-route-writer" });
    first.guidedToolJournal.start({
      turnId: "turn", callId: "result", toolName: "read_file",
      rawArguments: "{}", arguments: {},
    });
    first.guidedToolJournal.finish({
      callId: "result", status: "completed", result,
    });
    const replay = createOperationResultReplay({
      turnId: "turn", turnRevision: 1, journal: first.guidedToolJournal,
      exactReader: first.guidedOperationResultReader, exactReadCapability: true,
    });
    const admitted = replay.prepareMessages([message], "round", economics);
    ...
    const routed = createModelRoutePort({
      ...
      base: {
        ...
        async runRound(request) {
          sentBodies.push(openAIBoundedConversationSerializedBytes(
            request.messages,
            request.butlerData,
          ));
          physicalAttempt += 1;
      ...
      recordAcceptedResponse: async () => undefined,
    });
    const response = await routed.runRound({
      roundId: "round", model: "openai/gpt-5.6-sol", messages: admitted,
      tools: [], butlerData,
    });
    expect(sentBodies).toHaveLength(2);
    expect(new Set(sentBodies).size).toBe(1);
    replay.accepted("round", response);
    first.close();

    const resumed = openBtccSqliteStores({ dbPath, ownerId: "media-route-reader" });
    const later = createOperationResultReplay({
    ...
    }).prepareMessages([message], "later", economics);
    expect(later[0]?.operationResultReference?.version)
      .toBe("butler.operation-result-reference.v1");
    resumed.close();
  } finally {
...
test("enabled replay stays raw through routed retry, acknowledges once, and resumes route checkpoints", async () => {
  ...
  const routed = createModelRoutePort({
    ...
    base: {
      ...
      if (physicalCalls === 1) return {
        text: `private-analysis-${"A".repeat(5_000)}`,
        toolCalls: [{
  ...
  try {
    ...
    expect(physicalMessages[3]).not.toContain("butler.phase-continuity-projection.v1");
