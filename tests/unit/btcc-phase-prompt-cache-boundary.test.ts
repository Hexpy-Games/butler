...
describe("BTCC phase prompt cache boundary", () => {
  test("keeps changing capability and output carriers in the complete dynamic suffix", async () => {
    ...
    expect(calls[0]!.responseSchema).not.toEqual(calls[1]!.responseSchema);
    expect(calls[0]!.carrierFunctions).not.toEqual(calls[1]!.carrierFunctions);
    expect(JSON.stringify(calls[0]!.responseSchema)).not.toContain("result:read");
