...
- 원이 여전히 넓다는 사용자 피드백에 따라 배경만 20px/24px로 축소한다.
- 태스크: 크기 계약/기존 smoke 기대값 갱신 → CSS 토큰 변경 → 빌드 및
  4폭 실제 UI 검증. 클릭 영역 30px/44px, 아이콘, 이벤트는 변경하지 않는다.
- 완료: 빌드와 1440/800/390/320px smoke 통과. 배경 20px/24px 및 기존
  hit box를 실측했고, 원 밖 투명 측면에서도 메뉴가 열렸다. 데스크톱 스크린샷 검토 완료.
