...
export const enUsCopy: AppCopy = {
  projectStatistics: {
    ...
    aging: "Work to revisit", agingHelp: "Time since the last update, not time spent working or waiting.",
    calendar: "Project activity",
    activeItems: (count) => `${count} items in total`,
    workOverview: "Work overview", loading: (days) => `Loading the last ${days} days…`,
    refreshing: "Updating statistics…", loadFailed: "No statistics response was received. Check your connection and try again.",
    focus: "Where activity went",
    materials: "Materials and results", materialsHelp: "Documents registered or updated each day, and newly published assistant attachments. Repeated daily updates count once per document.",
    outcomes: "Butler request outcomes", outcomesHelp: "Requests with a recorded terminal event, grouped by that event's date. A completed reply does not mean a completed project task.",
    ...
    labels: { types: "By type", changes: "Changes", spec: "Specifications", plan: "Plans", report: "Reports",
      created: "Registered", completed: "Completion recorded", executed: "Execution completed", updated: "Updated",
      artifacts: "Published attachments", conversations: "Conversations", work: "Work changes", materials: "Materials",
      planned: "Planned", active: "In progress", review: "In review", blocked: "Blocked", other: "Other / unconfirmed",
      delivered: "Reply completed", failed: "Failed", cancelled: "Stopped", under30s: "Under 30 sec", under2m: "30 sec–2 min",
      under10m: "2–10 min", over10m: "10 min or more", recent: "0–7 days", older: "8–30 days", oldest: "31+ days", unknown: "Date unknown" },
  },
  projectSignpost: {
    ...
    briefingCoverage: (works, total, documents, reports, excluded) => `${works} of ${total} work records · ${documents} document titles · ${reports} recent report excerpts · ${excluded} candidate units excluded (not the entire conversation history)`,
    conversationTarget: "Conversation target", newProjectConversation: "New project conversation", pendingConversation: "Previously created conversation",
    sourceQuestion: "What should I look into next based on this source? ",
    missingSource: "This pinned source is unavailable.",
    loaded: (loaded, total) => `Showing ${loaded} of ${total}`,
    results: "Published results", noResults: "No results have been published in project conversations yet.",
...
rowMenu: (title) => `${title} menu`, organized: (title) => `Organized into ${title}.`, relocationDescription: (source, target) => `Move ${source} to ${target}. Conversation history and existing files are preserved. Future work will use the new workspace and project knowledge. Active work must finish before moving.`,
  ...
  settings: {
    ...
    localModels: {
      ...
      apiKeyDescription: "Enter a key if the server requires authentication. Leave untouched to keep the saved key. Re-enter it when changing the API URL.",
      apiKeyPlaceholder: "Enter a key or keep the saved key",
      apiKeyRemoved: "Key will be removed on save",
      removeApiKey: "Remove API key",
      title: "Custom",
      description:
        "Connect local servers or other providers using an OpenAI-compatible Chat Completions API. Enter a model ID directly if discovery is unavailable.",
      provider: "Provider",
      ...
      registeredLocalModels: "Registered Custom models",
      customOpenAiCompatible: "Custom OpenAI-compatible",
      discoveredStatus: (count) => `Found ${count} Custom models.`,
      ...
      deleteConfirm: (modelName) =>
        `Delete ${modelName} from Custom models?`,
      ...
      errors: {
        discover: "Failed to discover Custom models",
        register: "Failed to register Custom model",
        update: "Failed to update Custom model",
        delete: "Failed to delete Custom model",
      },
    },
    modelManagement: {
