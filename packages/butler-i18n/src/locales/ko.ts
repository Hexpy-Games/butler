...
rowMenu: (title) => `${title} 메뉴`, organized: (title) => `${title} 그룹으로 정리했습니다.`, relocationDescription: (source, target) => `${source} 대화를 ${target}(으)로 이동합니다. 대화 기록과 기존 파일은 보존되며, 이후 작업은 새 소속의 작업 폴더와 프로젝트 지식을 사용합니다. 진행 중인 작업이 있으면 완료한 뒤 이동할 수 있습니다.`,
  ...
  settings: {
    ...
    localModels: {
      apiKey: "API 키 (선택)",
      apiKeyDescription: "인증이 필요한 서버의 키를 입력하세요. 편집 시 입력하지 않으면 기존 키를 유지합니다. API 주소를 변경하면 키를 다시 입력하세요.",
      apiKeyPlaceholder: "키 입력 또는 기존 키 유지",
      apiKeyRemoved: "저장하면 키가 제거됩니다",
      removeApiKey: "API 키 제거",
      title: "Custom",
      description:
        "로컬 서버나 외부 제공자의 OpenAI 호환 Chat Completions API를 등록합니다. 모델 검색 없이 ID를 직접 입력할 수도 있습니다.",
      provider: "제공자",
      ...
      displayName: "표시 이름",
      maxContext: "최대 컨텍스트",
      registeredLocalModels: "등록된 Custom 모델",
      customOpenAiCompatible: "사용자 지정 OpenAI 호환",
      discoveredStatus: (count) => `Custom 모델 ${count}개를 찾았습니다.`,
      ...
      deleteConfirm: (modelName) =>
        `${modelName} 모델을 Custom 모델에서 삭제할까요?`,
      ...
      errors: {
        discover: "Custom 모델 검색 실패",
        register: "Custom 모델 등록 실패",
        update: "Custom 모델 업데이트 실패",
        delete: "Custom 모델 삭제 실패",
      },
    },
    modelManagement: {
      title: "모델 관리",
