/** Dashboard and navigation-space wire contracts. */
import type {
  ProjectSummary,
...
export interface NavigationView {
  ...
  automations_summary: { total_count: number; enabled_count: number };
  settings_summary: { profile_label: string };
  generated_at: string;
}
...
export interface DashboardBriefingSource {
  sourceId: string;
  kind: "work" | "plan" | "spec" | "report" | "message";
  id: string;
  revision: string;
  title: string;
  sessionId?: string;
...
export interface DashboardBriefingView {
  ...
  content?: DashboardBriefingContent;
  generatedAt?: string;
}
...
export interface DashboardStatisticsView {
  ...
  work: null | {
    work: DashboardStatisticSeries;
    task: DashboardStatisticSeries;
...
export interface DashboardBoardCard {
  ...
  status: string;
  lane: "planned" | "active" | "review" | "blocked" | "done" | "other";
  updatedAt: string;
  taskProgress: { done: number; total: number } | null;
...
export type DashboardHistoryPage =
  | {
      ...
      events: Array<{
        id: string;
        at: string;
        action: "created" | "updated" | "completed" | "reported" | "reviewed" | "disposition" | "result";
        workId?: string;
        session?: { id: string; title: string };
...
export interface ProjectDashboardView {
  ...
  activity: { days: ProjectDashboardActivityDay[] };
