...
export function classifyAppForegroundActiveWork({
  ...
  for (const chat of Array.isArray(navigation?.chats) ? navigation.chats : []) {
    ...
    if (state && !terminalTurnStates.has(state)) reasons.push("active_turn");
  }
  ...
  for (const worker of workers) {
    ...
    if (!state || !terminalWorkerStates.has(state)) reasons.push("active_worker");
  }
...
function activeWorkResult(classification, reasons) {
  return {
    classification,
    reasons,
