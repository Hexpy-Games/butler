...
- Treat Typo as structure only: it owns size, weight, line-height, letter spacing, and margin reset, but it must inherit color from the parent primitive, block, or app surface.
...
- Do not set text color inside Typo variants. Put color on the owning primitive, block, or domain surface so nested text like work decision bodies can inherit the intended tone.
