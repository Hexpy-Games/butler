...
export function useProjectBoard(projectId: string, kind: DashboardBoardCard["kind"], parent?: string, sourceRevision?: string) {
  const [page, setPage] = useState<DashboardBoardPage | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);
  const [revision, setRevision] = useState(0);
  const [cursor, setCursor] = useState<string | undefined>();
  const key = `${projectId}\0${kind}\0${parent ?? ""}\0${sourceRevision ?? ""}`;
  const windowKey = `board:${kind}:${parent ?? ""}`;
  ...
  useEffect(() => {
    ...
    const query = new URLSearchParams({ kind, limit: "50", ...(parent ? { parent } : {}), ...(cursor ? { cursor } : {}) });
    const request = async () => {
      let next = await api<DashboardBoardPage>(`/projects/${encodeURIComponent(projectId)}/dashboard/records?${query}`);
      const wanted = useProjectDashboardState.getState().projects[projectId]?.loadedCounts?.[windowKey] ?? 50;
