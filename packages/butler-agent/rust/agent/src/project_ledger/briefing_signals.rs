    ...
    impl Fixture {
        fn new() -> Self {
            let root = std::env::temp_dir()
                .join(format!("butler-briefing-ledger-{}", uuid::Uuid::new_v4()));
            fs::create_dir_all(root.join("project-ledger/projects/ledger-alpha/work/W-OPEN"))
                .unwrap();
            fs::create_dir_all(root.join("project-ledger/projects/ledger-alpha/work/W-DONE"))
                .unwrap();
