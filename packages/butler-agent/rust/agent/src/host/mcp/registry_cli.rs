...
mod tests {
    ...
    #[test]
    fn routes_registry_family_but_leaves_standalone_server_command_alone() {
        for command in [
            vec!["mcp"],
            vec!["mcp", "list"],
            vec!["mcp", "add"],
        ...
        assert!(!recognizes(&[
            OsString::from("mcp"),
            OsString::from("--data"),
            OsString::from("/tmp/data"),
            OsString::from("serve")
        ]));
        assert!(!recognizes(&[OsString::from("status")]));
    }
}
