...
mod tests {
    ...
    async fn missing_generation_skips_and_phase_failure_remains_in_daily_report() {
        ...
        let service =
            ConfiguredCycleService::new(root.clone(), paths.clone(), Arc::new(PhaseFixture));
        ...
        std::fs::write(memory.join("active-generation.json"), b"{}")
            .expect("mark descriptor present");
        ...
        let summary =
            std::fs::read_to_string(root.join("cognition/consolidation/run-summary.jsonl"))
                .expect("read summary");
        assert_eq!(
    ...
    async fn cancellation_after_phase_blocks_next_admission() {
        ...
        let service =
            ConfiguredCycleService::new(root.clone(), paths, Arc::new(CancelsOnFirstPhase));
        let result = service
            .run(
