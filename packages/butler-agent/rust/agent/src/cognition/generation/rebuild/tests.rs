...
impl CognitionCoordinationHost for TestHost {
    ...
    fn now_epoch_millis(&self) -> i64 {
        self.clock_reads.fetch_add(1, AtomicOrdering::Relaxed);
        SystemTime::now()
...
async fn cancellation_while_waiting_for_write_gate_removes_staged_snapshot() {
    ...
    let coordinator = Arc::new(CognitionWriteCoordinator::new(host).unwrap());
    ...
    .await
    .expect("snapshot must stage before cancellation");
...
async fn readiness_rejects_candidate_change_while_waiting_for_commit_gate() {
    ...
    let held = coordinator
        ...
        .unwrap()
        .unwrap();
    let clock_before = host.clock_reads.load(AtomicOrdering::Relaxed);
    let cancellation = CancellationToken::new();
    ...
    let change_after_compute = async {
        tokio::time::timeout(Duration::from_secs(10), async {
            loop {
                if host.clock_reads.load(AtomicOrdering::Relaxed) > clock_before {
                    break;
                }
                tokio::time::sleep(Duration::from_millis(20)).await;
            }
        ...
        .await
        .expect("read-only readiness must finish before gate acquisition");
...
async fn representative_commit_rejects_changed_candidate_after_gate_wait() {
    ...
    let change = async {
        tokio::time::timeout(Duration::from_secs(10), async {
            while host.clock_reads.load(AtomicOrdering::Relaxed) <= clock_before {
                tokio::time::sleep(Duration::from_millis(20)).await;
            }
        })
        .await
        .expect("commit must wait for gate");
