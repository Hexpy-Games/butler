...
mod tests {
    ...
    use crate::cognition::extraction::CandidateSearchFuture;

    struct GoldenSearch(Vec<ExtractCandidate>);
    impl CognitionCandidateSearch for GoldenSearch {
        fn search<'a>(&'a self, _input: CandidateSearchInput<'a>) -> CandidateSearchFuture<'a> {
            Box::pin(async move { Ok(self.0.clone()) })
        }
    }

    #[tokio::test]
    async fn binding_prompt_and_repair_schema_match_bun_source() {
        let golden: Value =
            serde_json::from_str(include_str!("binding/fixtures/binding-source-bun.json")).unwrap();
        let meaning: Meaning = serde_json::from_value(json!({"status":"processed",
            "entities":[{"name":"abc","evidence":[0]}],
            "items":[{"kind":"change","subject":0,"field":"deadline","old":"2026","new":"2027","evidence":[0]}],
            "attributes":[]})).unwrap();
        let passages = vec![Passage {
            id: 0,
            text: "abc deadline 2027".into(),
            quote: QuoteRef {
                unit_ref: "current".into(),
                quote: "abc deadline 2027".into(),
                occurrence: 0,
            },
            before: None,
            after: None,
        }];
        let candidates: Vec<ExtractCandidate> = serde_json::from_value(json!([
            {"ref":"entity-abc","type":"entity","label":"abc","aliases":[],"scope":"user","project_id":null,"claim":null,
             "evidence":[{"ref":"old-entity","text":"abc was tracked.","observed_at":"2026-01-01T00:00:00Z","basis":"user_statement"}]},
            {"ref":"claim-deadline","type":"memory_atom","label":"abc deadline 2026","aliases":[],"scope":"user","project_id":null,
             "claim":{"statement":"abc deadline 2026","subject_ref":null,"object_ref":null,"relation":null,"polarity":"positive","condition":null},
             "evidence":[{"ref":"old-claim","text":"abc deadline 2026.","observed_at":"2026-01-01T00:00:00Z","basis":"user_statement"}]}
        ])).unwrap();
        let search = GoldenSearch(candidates);
        let (batches, all) = prepare(
            &meaning,
            &passages,
            &search,
            &CandidateSearchInput {
                source_root: std::path::Path::new("unused"),
                generation_id: "g",
                embedding: None,
                cue: "",
        ...
        .unwrap();
        assert_eq!(
            json!(batches.iter().map(|b| &b.prompt).collect::<Vec<_>>()),
            golden["prompts"]
        );
        assert_eq!(
            json!(batches.iter().map(repair_schema).collect::<Vec<_>>()),
            golden["repairs"]
        );
        assert_eq!(
            json!(all.iter().map(|c| &c.ref_id).collect::<Vec<_>>()),
            golden["candidateRefs"]
        );
    }
