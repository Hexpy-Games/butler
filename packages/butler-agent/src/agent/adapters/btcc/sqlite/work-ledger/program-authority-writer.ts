...
export class SqliteProgramAuthorityWriter {
  ...
  bindProgram(mutation: BindProgram): void {
    ...
    if (binding.source === "deferred_goal") {
      this.bindDeferredProgram(mutation, scopeId);
      return;
    }
    this.db.query(`
  ...
  installReviewedPlan(product: InstallPlan["product"]): void {
    ...
    this.activateFirstPlan(product);
  }

  private bindDeferredProgram(mutation: BindProgram, scopeId: string): void {
    const authority = mutation.product.authority;
    const binding = authority.managedBinding;
    ...
    const rebound = this.db.query(`
      UPDATE btcc_programs SET authority_ref = ?, manifest_revision = manifest_revision + 1
  ...
  private activateFirstPlan(product: InstallPlan["product"]): void {
    const candidate = product.candidate;
  ...
  private continueReviewedPlan(
    ...
    if (candidate.plan.ref.id !== current.acceptedPlanRef) {
      this.replaceContinuedPlan(product);
      return;
    }
  ...
  private replaceContinuedPlan(product: InstallPlan["product"]): void {
    const candidate = product.candidate;
    this.closeUnacceptedAttempts(candidate.programId);
