...
import {
  continuationBudgetConfigurationMessage,
  continuationBudgetTerminalMessage,
  ...
  private async runAgentAndCommit(
    ...
    try {
      ...
      try {
        ...
        if (error instanceof TurnContinuationBudgetStorageError) throw error;
        permit.assertActive();
        result = isTurnContinuationBudgetExhaustedError(error)
          ? {
              route: "assisted",
              content: continuationBudgetTerminalMessage(
                turn.originalMessage,
                error,
              ),
            }
          : error instanceof TurnContinuationBudgetConfigurationError
            ? {
                route: "assisted",
                content: continuationBudgetConfigurationMessage(
                  turn.originalMessage,
                ),
              }
            : {
