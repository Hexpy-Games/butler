export const FEEDBACK_LOOP_DUTIES = {
  ...
  classify_correction_kind: [
    ...
    "A persistent correction from a read_only current Task likewise requires governing",
    "revision; never reinterpret Composer full_access as permission to widen that Task.",
  ...
  author_scoped_correction: [
    "Author only records required by the accepted CorrectionScope and preserve unaffected",
    "history. Declare executionRequirement as observation_only when correction produces no",
    "persistent delta, or workspace_mutation with the exact workspaceScopeRef and contained",
    "writablePaths. The requirement may narrow current Task authority but never widen it.",
  ].join(" "),
  ...
  review_correction_exactly: [
    ...
    "Reject an accepted implementation_repair when its workspace_mutation requirement is",
    "outside currentTask.artifactPolicy, including every read_only Task; select",
    "feedback_intent so governing Planning can reopen the true owning Task.",
