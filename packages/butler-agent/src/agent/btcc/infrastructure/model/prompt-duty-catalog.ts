...
const DUTIES = {
  ...
  review_executability: "Require every Task to have sufficient inputs, authority, target, criteria, and verification to execute and review independently.",
  ...
  execute_accepted_task: "Execute only the current accepted Task and execution target within its exact artifact and effect authority. The operationAuthority declares the provisioned operation root: a single-file workspace accepts only relativeTarget and tool path 'target'; a directory workspace uses '.' as its complete root and contained paths. Never prepend artifactPolicy.targetPath or write another Task's target.",
  ...
  review_task_independently: "Compare the immutable Task result and current targets with every accepted criterion and verification question; keep validation disposable and never mutate the reviewed source.",
  ...
  classify_correction_kind: "Choose implementation repair, governing revision, authority-scope revision, or pre-plan repair from semantic cause, never runtime heuristics.",
  ...
  render_final_dossier_truthfully: "Render only the accepted FinalDossier disposition, outcomes, receipts, open frontier, and truthful limitations; never repair or reinterpret it.",
...
const PROHIBITIONS = {
  ...
  no_generic_evidence: "Do not invent an Evidence layer; use concrete domain records and observations.",
