...
const DUTIES = {
  ...
  review_task_independently: "Compare the immutable Task result and current targets with every entry in stateInput.criteria and its verification questions. Return exactly one verdict per entry, copy each criterion ref byte-for-byte, and never introduce criteria from another Task. Keep validation disposable and never mutate the reviewed source.",
