...
const DUTIES = {
  ...
  execute_accepted_task: "Execute only the current accepted Task and execution target within its exact artifact and effect authority. A provisioned workspace root is the complete planned target: tool paths and relativeTarget are inside that root, use '.' for the whole directory, and never prepend artifactPolicy.targetPath.",
