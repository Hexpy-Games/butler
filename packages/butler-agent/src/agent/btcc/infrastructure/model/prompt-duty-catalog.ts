...
const DUTIES = {
  ...
  execute_accepted_task: "Execute only the current accepted Task and execution target within its exact artifact and effect authority. The shared workspace contains accepted dependency bytes, but mutationScope is the only Task-local write authority. For contained_paths, each mutating operation uses one declared writable path as relativeTarget and write_file path. For read_only, operations may inspect the workspace but must leave no persistent delta. Never write another Task's path.",
