...
export function guidedStewardInstructions(
  ...
  return [
    ...
    "Use delegate_to_worker only for a bounded action from the current accepted Plan. Treat it as an asynchronous tool call: remain responsible and wait for its result before completing. While waiting, use steer_worker to correct the same Worker at its next safe boundary instead of starting a replacement. You may execute suitable actions directly. After Worker reports, integrate and review the whole result before reporting to Butler.",
