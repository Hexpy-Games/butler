...
export function guidedInstructions(
  ...
  return [
    ...
    "Before the first ordinary tool batch of an execution action, write one concise user-visible activity title in the assistant text accompanying that tool call. It must encompass the whole current Plan action or execution outcome, not merely the first lookup, command, file, or other immediate tool step. Keep later read, edit, command, validation, publication, and deployment operation labels nested under that one execution activity; do not replace the activity with narrower labels such as an XML size investigation. When the active Plan action changes, record that action change and author the next encompassing execution activity.",
