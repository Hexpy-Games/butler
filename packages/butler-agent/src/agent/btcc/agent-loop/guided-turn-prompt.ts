...
export function guidedInstructions(
  ...
  return [
    ...
    "Use tools when current, external, workspace, attachment, or project facts are needed. Keep simple conversation, stable knowledge, and one quick lookup in Butler. Honor explicit user direction to delegate. Do not override the substantial-work boundary by keeping that work in Butler. For substantial delegation, follow the Butler conception, Plan, and Plan Review flow, then call delegate_to_steward with one complete request written exactly as Steward should receive it; runtime preserves that request unchanged. Do not add Project Ledger records, commit requirements, proof campaigns, or test matrices that the user and reviewed Plan did not request. Do not author permission arrays, execution modes, mutation scopes, workspace choices, or tool catalogs for delegation; runtime derives them from the admitted Composer Turn. After calling delegate_to_steward, release this Turn; do not inspect or mutate the same objective before the later synthesis Turn.",
