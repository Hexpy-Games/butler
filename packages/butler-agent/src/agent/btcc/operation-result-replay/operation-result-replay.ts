...
export function operationResultReplayEnabled(
  ...
  return TRUE_FLAG_VALUES.has(
    (env.BUTLER_OPERATION_RESULT_REPLAY ?? "").trim().toLowerCase(),
  );
}
...
export type OperationResultReplay = {
  prepareMessages(
    ...
    roundId: string,
    economics?: OperationResultReplayEconomics,
  ): ModelRoundMessage[];
...
export type OperationResultReplayEconomics = {
    ...
    prepareMessages(messages, roundId, economics) {
      return messages.map((message) => {
        ...
        if (!record.deliveryState) {
          if (!referenceSavesProviderMessageBytes(
            message,
            referenceContent,
            economics,
          )) {
...
function referenceSavesProviderMessageBytes(
  raw: ModelRoundMessage,
  referenceContent: string,
  economics?: OperationResultReplayEconomics,
): boolean {
  const reference: ModelRoundMessage = {
    ...raw,
    content: referenceContent,
    requestSegmentKind: "older_tool_result_projection",
  };
  if (economics?.statelessMessageBytes) {
    return economics.statelessMessageBytes([reference], economics.butlerData) <
      economics.statelessMessageBytes([raw], economics.butlerData);
  }
  return Buffer.byteLength(JSON.stringify(referenceContent), "utf8") <
    Buffer.byteLength(JSON.stringify(raw.content), "utf8");
}
