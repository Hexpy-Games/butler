...
const COMPACT_CONTRACTS = [
  ...
  },
  {
    ...
    applicableRules: [
      ...
      "Artifact targets are explicit contained workspace-relative paths. Use the exact admitted root '.' only when one cohesive multi-file outcome requires that root; never widen to it by convenience.",
      "A Task that observes or extends a predecessor WorkspaceRevision remains workspace_artifact on the exact same target; non_artifact Tasks cannot materialize isolated dependency bytes.",
