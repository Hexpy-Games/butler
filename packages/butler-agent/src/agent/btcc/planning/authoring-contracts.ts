...
const COMPACT_CONTRACTS = [
  {
    ...
    applicableRules: [
      ...
      "A stopped continuation Plan revision must carry the exact interrupted Task record while its ResultCandidate remains reviewable. Its inner GoalContract and governing refs may therefore remain the accepted stopped revisions even when the outer continuation candidate binds a fresh coherent Goal. Do not require rebinding or reauthoring that Task; changing its ref would invalidate the retained ResultCandidate. Revise only surrounding Plan records or explicit successor work justified by the stopped Review.",
      "Accepted historical Tasks remain immutable history, not evergreen proof that mutable repository, remote, deployment, or Ledger postconditions still hold. During continuation Planning, reconcile fresh observations with every carried predecessor required by the remaining frontier. When current state contradicts an accepted predecessor criterion, preserve the historical record and author the smallest explicit successor repair or reconciliation Task; never silently treat the stale postcondition as satisfied or reuse the old Task identity.",
      ...
      "Bind every external_target_mutation EffectIntent to exactly one requiredTargetEffect from the GoalContract by requiredTargetEffectId. Preserve its exact targetScopeRef and desiredOutcome. Cover every requiredTargetEffect exactly once; repository promotion is runtime-owned and binds none.",
      ...
      "Repository promotion, local Git commit, remote push, and deployment are distinct effects. A promotion Task has effectClass repository_promotion and owns only the atomic reviewed-byte exchange. If the Goal requires commit, push, or deployment, author separate ordered external_effect Tasks with exact non-workspace targets and external_target_mutation EffectIntents; never give those outcomes or criteria to the promotion Task.",
      ...
      "Each external_effect Task has one external_target_mutation EffectIntent; a repository_promotion Task has exactly one repository_promotion EffectIntent and no other Task owns it.",
