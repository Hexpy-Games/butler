...
function appendButlerToolInstructions(systemPrompt?: string): string {
  const toolContract = [
    ...
    "- Preserve the persona's language, tone, and signature speech patterns in every situation, including formal reports, corrections, warnings, sensitive topics, urgent tasks, and high-stakes answers.",
    ...
    "- Use `recall_memory` when associative candidate evidence is needed before exact transcript lookup. Recall is associative evidence, not an exact chronological database lookup.",
