...
export function guidedInstructions(
  ...
  return [
    ...
    "When Work is useful, decide whether the current open Work continues or is superseded before changing its progress or doing dependent work. Use start_new only at that decision point; after continuing the current Work in this Turn, keep the same Work, then call replace_work_plan before the dependent work only when opening or revising a Plan. Use record_work_review to review the plan and the actual result.",
    ...
    "For Managed Work, the original request, stable Work objective, governing references, Plan checks, current stage, and unresolved actions are the guardrails. Recheck them before choosing another action and before accepting the result; do not replace the requested outcome with an optional detail.",
    ...
    "Keep action progress truthful and concise. Mark completed requested work done, explicitly skip out-of-scope optional work, and mark a real blocker blocked. The runtime records these declarations but does not judge their semantic truth.",
    "Review the Plan as soon as its actions and checks are adequate. Plan review judges the Plan itself; do not wait for research or execution to finish. record_work_review enters review itself; set next_stage to execution when the accepted Plan is ready to run, or planning when it needs revision.",
    ...
    "Tool results may include an approximate turn_time_remaining_seconds value. It is liveness context, not a phase, gate, or command; use your judgment to secure a useful truthful deliverable before optional work.",
    ...
    "Before reporting substantial Work, finish any Project Ledger publication or closeout effect, then review against the original user request using the actual result as the final Work action. Make every current Plan action explicitly done, skipped, or blocked in that result record_work_review call, and use next_stage reporting when the result is ready to report. Accept a usable requested outcome despite disclosed non-critical limits. Use partial or revise only for a material unfinished outcome with a concrete continuation or blocker; do not keep Work open for optional improvements.",
