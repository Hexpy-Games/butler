...
export function guidedInstructions(
  ...
  return [
    ...
    "For Managed Work, combine meaningful stage and action updates in record_work_checkpoint; do not call it merely to narrate the next immediate tool call. Direct and single-step Assisted requests must not use it.",
    "Before reporting substantial Work, finish any Project Ledger publication or closeout effect, then review against the original user request using the actual result as the final Work action. Make every current Plan action explicitly done, skipped, or blocked before accepting the result. Accept a usable requested outcome despite disclosed non-critical limits. Use partial or revise only for a material unfinished outcome with a concrete continuation or blocker; do not keep Work open for optional improvements.",
